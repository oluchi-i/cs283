...
int count_words(char *str) {
    ...
    word_start = false;
    for (i = 0; str[i] != '\0' ; i++) {
        if (word_start == false) { // Please implement
            if (str[i] == SPACE_CHAR) {
                continue; 
        }
            wc += 1;
            ...
            if (str[i] == SPACE_CHAR) {
                word_start = false; //checks for white space and counts 
            }
    }
}
return wc;
...
void  word_print(char *str){
    ...
    char curr;    //where curr is the current character.
...
int main(int argc, char *argv[]){
    ...
    switch (opt){
        ...
        case 'r':
            ...
            //ANSWER:
            //The string is reversed in place because our function tells the compiler to constantly swap only the first and
            //last index characters of the string, as long as the first index is less than the last index character.
            break;
        case 'w': 
            printf("Word Print\n----------\n");
        ...
        //          your thoughts.
        //ANSWER:
        //The default case here helps the program to catch invalid inputs from the user, e.g. any input other than
        //h,c,r,w will provide the error message in the default case.
        default:
            usage(argv[0]);
    ...
    //ANSWER:
    //The break statement is used in the switch block to prevent 
    //"fall-through," where multiple cases might execute unintentionally. 
    // you forget a break statement, the program would continue to execute 
    //the code for the next case(s), even if they don't match the user’s input.
}