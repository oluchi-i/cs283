...
int student_exists(int fd, int id);
...
int get_student(int fd, int id, student_t *s)
{
    ...
    printf("DEBUG: Scanning for ID: %d\n", id);

    // Read through the file to find the student by ID
    while (read(fd, &temp, RECORD_SIZE) == RECORD_SIZE) {
        if (temp.id != 0 && temp.id == id) {  
            *s = temp;

            // Output the full student info (first name, last name, GPA)
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    student_t student;
    student.id = id;
    strncpy(student.fname, fname, MAX_NAME_LEN);
    student.fname[MAX_NAME_LEN - 1] = '\0';  
    strncpy(student.lname, lname, MAX_NAME_LEN);
    student.lname[MAX_NAME_LEN - 1] = '\0';  
    student.gpa = gpa;
    ...
    if (read(fd, &existing_student, sizeof(student_t)) == sizeof(student_t) && existing_student.id == student.id) {
        printf(M_ERR_DB_ADD_DUP, id); 
        return ERR_DB_OP;
    }

    if (student_exists(fd, id)) {
        printf(M_ERR_DB_ADD_DUP, id);  
        return ERR_DB_OP;
    }
    ...
    if (write(fd, &student, sizeof(student_t)) != sizeof(student_t)) {
        printf("Error writing student to database.\n");
        return ERR_DB_FILE;
    }
...
int del_student(int fd, int id)
{
    ...
    while (read(fd, &record, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
        if (record.id == id) {
            record.id = DELETED_STUDENT_ID;
            lseek(fd, pos, SEEK_SET);
            write(fd, &record, STUDENT_RECORD_SIZE);
...
int count_db_records(int fd)
{
    ...
    while (read(fd, &s, RECORD_SIZE) == RECORD_SIZE) {
        if (s.id != 0) {
            record_count++;  
        }
        offset += RECORD_SIZE;
...
{
    // TODO
    int tmp_fd = open(TMP_DB_FILE, O_CREAT | O_WRONLY, S_IRUSR | S_IWUSR);
    if (tmp_fd == -1) {
        return ERR_DB_OP;
    }
    ...
    lseek(fd, 0, SEEK_SET);
    while (read(fd, &record, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) {
        if (record.id != DELETED_STUDENT_ID) {
            write(tmp_fd, &record, STUDENT_RECORD_SIZE);
        }
    }
